  ...
  Widget build(BuildContext context) {
    ...
    try {
      if (message.isTextType) {
        ...
        // Red Packet logic (Cleaned & Added Emoji)
        var data = message.customElem?.data;
        if (data != null) {
          try {
            final map = json.decode(data);
            var type = map['customType'];
            // Compatibility: Check nested data or alternate keys
            if (type == null && map['data'] is Map) {
              type = map['data']['customType'];
            }

            if (type == 'RedPacket' ||
                type == 'RedPacketClaim' ||
                map['type'] == 'RedPacket') {
              // 剑盾: Use 'text' (from RedPacketSendController) as primary source
              var msgContent = map['text'] ??
                  map['content'] ??
                  map['wish'] ??
                  map['greetings'];

              if (msgContent == null || msgContent.toString().trim().isEmpty) {
                msgContent = '恭喜发财'; // Default only if totally empty
              }
              // 剑盾: User preferred format (No label, just Emoji + Text)
              content = '🧧 $msgContent';
            }
          } catch (e) {
            // Ignore parse error
