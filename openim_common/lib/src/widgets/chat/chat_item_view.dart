...
class _ChatItemViewState extends State<ChatItemView> {
  ...
  bool get _isRedPacket {
    if (_message.isCustomType) {
      var data = IMUtils.parseCustomMessage(_message) ?? <String, dynamic>{};
      try {
        if (_message.customElem?.data != null) {
          var raw = json.decode(_message.customElem!.data!);
          if (raw is Map<String, dynamic>) {
            data.addAll(raw);
          }
        }
      } catch (_) {}
      return data['viewType'] == 'RedPacket' ||
          data['type'] == 'RedPacket' ||
          data['packetType'] != null;
    }
    return false;
  }
