...
//            }
        ...
        if newSettingsMap["enableViewportScale"] != nil && settings?.enableViewportScale != newSettings.enableViewportScale {
            if !newSettings.enableViewportScale {
                if configuration.userContentController.userScripts.contains(ENABLE_VIEWPORT_SCALE_JS_PLUGIN_SCRIPT) {
                    ...
                    super.evaluateJavaScript(NOT_ENABLE_VIEWPORT_SCALE_JS_SOURCE, completionHandler: nil)
                }
            } else {
                super.evaluateJavaScript(ENABLE_VIEWPORT_SCALE_JS_SOURCE, completionHandler: nil)
        ...
        if newSettingsMap["supportZoom"] != nil && settings?.supportZoom != newSettings.supportZoom {
            if newSettings.supportZoom {
                if configuration.userContentController.userScripts.contains(NOT_SUPPORT_ZOOM_JS_PLUGIN_SCRIPT) {
                    ...
                    super.evaluateJavaScript(SUPPORT_ZOOM_JS_SOURCE, completionHandler: nil)
                }
            } else {
                super.evaluateJavaScript(NOT_SUPPORT_ZOOM_JS_SOURCE, completionHandler: nil)
        ...
        evaluateJavascript(source: flagVariable) { (alreadyLoaded) in
            if let alreadyLoaded = alreadyLoaded as? Bool, alreadyLoaded {
                ...
                if #available(iOS 14.0, *), pluginScript.requiredInAllContentWorlds {
                    ...
                    self._evaluateJavaScriptCompat(enableSource, completionHandler: nil)
                }
                if !enable {
                ...
                if #available(iOS 14.0, *), pluginScript.requiredInAllContentWorlds {
                    ...
                    self._evaluateJavaScriptCompat(pluginScript.source, completionHandler: nil)
                    self.configuration.userContentController.addPluginScript(pluginScript)
                }
    ...
    public func injectDeferredObject(source: String, contentWorld: WKContentWorld, withWrapper jsWrapper: String?, completionHandler: ((Any?) -> Void)? = nil) {
        ...
        evaluateJavaScript(jsToInject, frame: nil, contentWorld: contentWorld, completionHandler: { (evalResult) in
            guard let completionHandler = completionHandler else {
                return
            }
    ...
    public func _evaluateJavaScriptCompat(_ javaScriptString: String, completionHandler: ((Any?, Error?) -> Void)? = nil) {
        if let applePayAPIEnabled = settings?.applePayAPIEnabled, applePayAPIEnabled {
    ...
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        _evaluateJavaScriptCompat(PLATFORM_READY_JS_SOURCE, completionHandler: nil)
...
""", completionHandler: nil)
    ...
    public func pauseTimers() {
        if !isPausedTimers {
            isPausedTimers = true
            let script = "alert();";
            self._evaluateJavaScriptCompat(script, completionHandler: nil)
        }
    }
