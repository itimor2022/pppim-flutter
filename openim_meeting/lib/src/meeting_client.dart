...
class MeetingClient implements MeetingBridge {
  ...
  close() async {
    ...
    try {
      ...
      Logger.print('Failed to disable wakelock: $e');
    }
  }
  ...
  _connect(
    ...
    try {
      ...
      try {
        await LoadingView.singleton.wrap(asyncFunction: () async {
          ...
          try {
            ...
            Logger.print('Failed to enable wakelock: $e');
          }
      ...
      if (errorMsg.contains('NotExist')) {
        IMViews.showToast(StrRes.meetingIsOver);
      } else if (errorMsg.contains('permission not granted')) {
        IMViews.showToast('请开启摄像头和麦克风权限');
      } else {
        // 在网络异常的基础上增加具体错误提示，方便定位
        IMViews.showToast("${StrRes.networkError} ($errorMsg)");
      }
    }
  }
