...
class ChatLogic extends GetxController {
  ...
  /// 转发
  /// 转发
  /// [isMergeForward] 是否合并转发
  void forward(Message? message, {bool isMergeForward = false}) async {
    final result = await AppNavigator.startSelectContacts(
      action: SelAction.forward,
      ex: null != message
      ...
      for (var info in checkedList) {
        ...
        if (null != message) {
          ...
          if (isMergeForward) {
            ...
            for (var msg in multiSelList) {
              sendForwardMsg(msg, userId: userID, groupId: groupID);
            }
          }
        }
      }
  ...
  // 为了让全员禁言时能发指令，此处 isMuted 仅对单人禁言生效，全员禁言通过拦截发送实现
  bool get isMuted => isUserMuted /* || isInBlacklist.value*/;
  ...
  Future<AdvancedMessage> _requestHistoryMessage() async {
    ...
    if (result.messageList != null) {
      for (var msg in result.messageList!) {
        // [FIX] Hydrate exMap from localEx for Red Packet persistence
        if (msg.localEx != null && msg.localEx!.isNotEmpty) {
          try {
            var map = json.decode(msg.localEx!);
            if (map is Map<String, dynamic>) {
              msg.exMap.addAll(map);
            }
          } catch (e) {
            // ignore
          }
  ...
  Future<bool> onScrollToBottomLoad() async {
    try {
      ...
      // [FIX] Catch error to stop loading spinner
      return false;
    }
  }
...
// }
  ...
  void redPacketCheck(Map redPacketData, String msgID) async {
    final packetID = redPacketData['packet_id'] ?? redPacketData['id'];
    if (packetID == null) {
      IMViews.showToast("Unknow Red Packet");
      return;
    }

    try {
      LoadingView.singleton.wrap(asyncFunction: () async {
        final detail = await RedPacketApi.getRedPacketDetail(
            packetID: packetID, userID: OpenIM.iMManager.userID);
        if (detail == null) return;

        // Proto keys are usually camelCase in Dart/JS unless configured otherwise suitable for proto3 json
        // We handle potential key naming diffs gracefully or assume camelCase for now based on typical OpenIM usage
        // Note: Proto3 JSON omits default values (0). So if missing, it means 0.
        final remainCount =
            detail['remainCount'] ?? detail['remain_count'] ?? 0;
        final status = detail['status']; // 1:Active, 2:Finished, 3:Expired
        final type = detail['type'];
        final records = (detail['records'] as List?) ?? [];
        final recvID = detail['recvId'] ?? detail['recv_id'];

        // Sender info usually depends on message content if not in detail
        final senderNickname = redPacketData['senderNickname'] ?? "";
        final senderFaceUrl = redPacketData['senderFaceUrl'] ?? "";
        final content = redPacketData['data'] ?? "恭喜发财";

        // Check if I already grabbed it
        bool amIGrabbed = records.any(
            (r) => (r['userId'] ?? r['user_id']) == OpenIM.iMManager.userID);

        if (amIGrabbed) {
          _updateMessageRedPacketStatus(msgID, 'opened');
          Get.toNamed(AppRoutes.redPacketRecords, arguments: packetID);
          return;
        }

        // Exclusive Red Packet (Type 3)
        if (type == 3) {
          if (status == 2 || (remainCount != null && remainCount <= 0)) {
            _updateMessageRedPacketStatus(msgID, 'finished');
          } else if (status == 3) {
            _updateMessageRedPacketStatus(msgID, 'expired');
          }

          if (recvID == OpenIM.iMManager.userID) {
            // I am receiver
            if (status == 1) {
              Get.dialog(OpenRedPacketDialog(
                content: content,
                nickname: senderNickname,
                headimg: senderFaceUrl,
                id: packetID,
                msgId: msgID,
                onReceive: _grabRedPacket,
                onViewDetails: () => Get.toNamed(AppRoutes.redPacketRecords,
                    arguments: packetID),
              ));
            } else {
              // Not active -> Details
        ...
        // Normal/Lucky Packet
        if (status == 2 || (remainCount != null && remainCount <= 0)) {
          // Finished -> HandSlowDialog
          // Update local message status to 'finished'
          _updateMessageRedPacketStatus(msgID, 'finished');

          Get.dialog(HandSlowDialog(
          ...
          // Expired -> Details (or Expired Dialog if needed, but usually Details)
          // Update local status too? Maybe 'expired'.
          _updateMessageRedPacketStatus(msgID, 'expired');
          Get.toNamed(AppRoutes.redPacketRecords, arguments: packetID);
        } else {
          // Active -> Open Dialog
          ...
          bool isSender =
              (detail['sender_id'] ?? redPacketData['sendID']) == currentUserID;
      ...
      IMViews.showToast("获取红包失败: $e");
    }
  }

  Future<void> _grabRedPacket(String packetID, String msgID) async {
    try {
      await RedPacketApi.grabRedPacket(
          packetID: packetID, userID: OpenIM.iMManager.userID);

      // Sync status to opened
      final m = messageList.firstWhereOrNull((e) => e.clientMsgID == msgID);
      if (m != null) {
        m.exMap['redPacketStatus'] = 'opened';
        OpenIM.iMManager.messageManager.setMessageLocalEx(
            conversationID: conversationInfo.conversationID,
            clientMsgID: msgID,
            localEx: json.encode(m.exMap));
        messageList.refresh();
      }
      ...
      Get.back(); // close dialog
      // If error indicates finished or other status, toast it.
      IMViews.showToast(e.toString());
      Get.toNamed(AppRoutes.redPacketRecords, arguments: packetID);
    }
  }

  void _updateMessageRedPacketStatus(String msgID, String status) {
    try {
      final m = messageList.firstWhereOrNull((e) => e.clientMsgID == msgID);
      if (m != null) {
        if (m.exMap['redPacketStatus'] == status) return;
        m.exMap['redPacketStatus'] = status;
        OpenIM.iMManager.messageManager.setMessageLocalEx(
            conversationID: conversationInfo.conversationID,
            clientMsgID: msgID,
            localEx: json.encode(m.exMap));
      ...
      print("Update RedPacket status error: $e");
    }
  }

  bool isRedPacket(Message message) {
    try {
      if (message.contentType == MessageType.custom) {
        var data = message.customElem!.data;
        var map = json.decode(data!);
        return map['type'] == "RedPacket" || map['packetType'] != null;
      }
    } catch (e) {}
    return false;
  }
